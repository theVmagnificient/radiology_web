...
from ..dataset import Dataset, Pipeline, F  # pylint: disable=no-name-in-module
...
def get_crops(nodules, fmt='raw', crop_size=(32, 64, 64), batch_size=20, share=0.5, histo=None,
    ...
    **kwargs
    ...
    args_sample_nodules = dict(crop_size=crop_size, batch_size=batch_size,
                               share=share, histo=histo, variance=variance)
...
def split_dump(cancer_path, non_cancer_path, nodules, histo=None, fmt='raw',
               crop_size=(32, 64, 64), variance=(36, 144, 144), **kwargs):
    ...
    **kwargs
    ...
    # set up args-dicts
    args_dump_cancer = dict(dst=cancer_path, n_iters=N_ITERS, crop_size=crop_size,
                            variance=variance, share=1.0, batch_size=None)
    args_sample_ncancer = dict(crop_size=crop_size, histo=histo,
...
def combine_datasets(datasets, batch_sizes, pipeline):
    """ Get pipeline for generating batches from datasets in given proportion.

    Parameters
    ----------
    datasets : dataset.Dataset or ArrayLike[dataset.Dataset]
        datasets that will be used batch generation.
    batch_sizes : int or ArrayLike[int]
        batch sizes for datasets that were passed a first argument.
        Note that length of this argument must match the length of 'datasets'
        argument.
    pipeline : dataset.Pipeline
        pipeline to apply to batches generated from each dataset.

    Returns
    -------
    dataset.Pipeline
        pipeline that generates batches containing items from datasets
        passed as first argument in chosen proportion.

    Raises
    ------
    ValueError
        if types of 'datasets', 'batch_sizes' and 'pipeline' arguments
        don't meet requirements mensioned in Parameters section.
    """
    if isinstance(datasets, Dataset):
        datasets = [datasets]
    elif (isinstance(datasets, (list, tuple, np.ndarray))
          and all(isinstance(item, Dataset)for item in datasets)):
        datasets = list(datasets)
    else:
        raise ValueError("Argument 'datasets' must be instance of 'dataset.Dataset' "
                         + "or array-like of 'dataset.Dataset instances")

    if isinstance(batch_sizes, (int, np.int, np.int32, np.int64)):
        batch_sizes = np.array([batch_sizes] * len(datasets))

    elif isinstance(batch_sizes, (list, tuple, np.ndarray)):
        batch_sizes = np.array(batch_sizes, dtype=np.int)
        if len(batch_sizes) != len(datasets):
            raise ValueError("Argument 'batch_sizes' must have the same"
                             + " length as argument 'datasets'.")

    if not isinstance(pipeline, Pipeline):
        raise ValueError("Argument 'pipeline' must be instance of"
                         + " class 'dataset.Pipeline'.")

    pipelines_list = []
    for idataset, ibatch_size in zip(datasets, batch_sizes):
        ipipeline = (idataset >> pipeline).run(lazy=True,
                                               batch_size=ibatch_size,
                                               n_epochs=None)
        pipelines_list.append(ipipeline)

    return Pipeline.merge(*pipelines_list)
...
def combine_crops(cancer_set, non_cancer_set, batch_sizes=(10, 10), hu_lims=(-1000, 400),
    ...
    ppl_cancer = (cancer_set.p
                  ...
                  .normalize_hu(min_hu=hu_lims[0], max_hu=hu_lims[1])
                  .run(lazy=True, batch_size=batch_sizes[0],
                       n_epochs=None, drop_last=True, shuffle=9)
                 )
    ...
    pipeline = (non_cancer_set.p
                ...
                .normalize_hu(min_hu=hu_lims[0], max_hu=hu_lims[1])
                .merge(ppl_cancer)
                .run(lazy=True, batch_size=batch_sizes[1],
                     n_epochs=None, drop_last=True, shuffle=9)
               )
...
@options_prod(angle=(-15, 15, -30, 30, -45, 45, -60, 60, -75, 75, -90, 90,
                     -105, 105, -130, 130, -145, 145, -165, 165, -180, 0))
...
def sample_and_rotate(crop_size=(32, 64, 64), histo=None, variance=(49, 169, 169),
                      angle=30, rot_axes=(1, 2), rate=176, max_crops=64,
    ...
    crop_size = np.rint(np.array(crop_size) * np.sqrt(2)).astype(np.int)
    batch_size = max_crops if not cancerous else None
    pipeline = (
        Pipeline()
        .sample_nodules(share=int(cancerous), crop_size=crop_size,
    ...
    if cancerous is None:
        return (
            Pipeline()
            .repeat_pipeline(pipeline, F(cancer_rate_to_num_repeats, rate=rate / 22))
        )
    else:
        return (
            Pipeline()
            .repeat_pipeline(pipeline, F(ncancer_rate_to_num_repeats,
                                         rate=rate / 22, max_crops=max_crops))
        )
...
def sample_simple(crop_size=(32, 64, 64), histo=None, variance=(49, 169, 169),
    ...
    """ Sample cancerous and non-cancerous crops.
