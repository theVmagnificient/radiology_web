...
#from viz_tools import calc_sensitivity_recall
...
class Estimator():
    ...
    def step(self, inputs, targets):
        ...
        targets : ndarray, torch Tensor or Variable
            targets of model.
        """
        inputs_ = self.as_torch(inputs,
                                device=(self._device
                                if not isinstance(self._device, list)
                                else self._device[0]),
                                grad=False)
        targets_ = self.as_torch(targets,
    ...
    def compute_metrics(self, inputs, targets, metrics, batch_size=None,
        ...
        self.model.eval()
        val_y = self.as_torch(targets,
                              dtype=str(targets.dtype),
                              device=(self._device
                              if not isinstance(self._device, list)
                              else self._device[0]), grad=False)
        # if `inputs` too large, use batch logic. i.e.
        # split-predict-gather and compute loss/metrics for entire `inputs`
        if batch_size:
            prediction_list = []
            for b_iter in tqdm(range(len(inputs) // batch_size +
                                     (1 if len(inputs) % batch_size else 0)),
    ...
    def compare_nodules(self, batch, nodules_true, nodules_pred):
        result = overlap_nodules(batch, nodules_true, nodules_pred)

        try:
            nb_pred = len(result['pred_stats'][0])
        except:
            nb_pred = 0
        print('Number of objects predicted by model is {}'.format(nb_pred))

        self._true_stats += [result['true_stats']]
        self._pred_stats += [result['pred_stats']]

    def show_segmentation_results(self):
        if len(self._pred_stats) > 1:
                self.pred_nods = pd.concat([pd.concat(e) for e in self._pred_stats if len(e) > 0]).reset_index()
        else:
            print('Nothing was predicted!')
        if len(self._true_stats) > 1:
            self.true_nods = pd.concat([pd.concat(e) for e in self._true_stats if len(e) > 0]).reset_index()
        else:
            print('Nothing was annotated!')
