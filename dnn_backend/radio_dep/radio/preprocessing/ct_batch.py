...
from .dump import encode_dump_array_sync, encode_dump_array_async, dump_dicom

from .augmentation import salt_and_pepper
from .exceptions import assert_component_is_loaded

from ..debug import DebugActions
...
class CTImagesBatch(Batch, metaclass=DebugActions):  # pylint: disable=too-many-public-methods
    ...
    def apply(self, callback, *args, **kwargs):
        """ Apply given callable to batch and return result.

        Parameters
        ----------
        callback : callable(batch, *args, **kwargs) -> batch
            function to apply.
        """
        ...
        **kwargs
        ...
        if fmt == 'ndarray':
            ...
            if kwargs.get('sync', False):
                self._load_blosc_sync(components=components)    # pylint: disable=no-value-for-parameter
            else:
                self._load_blosc_async(components=components)     # pylint: disable=no-value-for-parameter
        elif fmt == 'raw':
    ...
    async def _load_blosc_component_async(self, ix, component, ext):
        """ Load component stored in blosc format asynchronously.
        ...
        async with aiofiles.open(component_path, mode='rb') as file:
    ...
    @inbatch_parallel(init='_init_load_blosc', post='_post_default',
                      target='for', update=False)
    def _load_blosc_sync(self, ix, *args, **kwargs):
        ...
        **kwargs
        ...
        for component in kwargs['components']:
            ext = 'blk' if component in ['images'] else 'pkl'
    ...
    def _dump_blosc_component_sync(self, ix, component, ext, folder,
                                   index_to_name=None, i8_encoding_mode=None):
        """ Dump data on disk in specified folder.
        ...
        component_folder = os.path.join(folder, item_subdir, component)
        if not os.path.exists(component_folder):
            os.makedirs(component_folder)

        if ext == 'blk':
            if isinstance(i8_encoding_mode, dict):
                mode = i8_encoding_mode.get(component, None)
            else:
                mode = i8_encoding_mode

            encode_dump_array_sync(self.get(ix, component),
                                   component_folder,
                                   'data.blk', mode)

        elif ext == 'pkl':
            byted = pickle.dumps(self.get(ix, component))
            with open(os.path.join(component_folder, 'data.pkl'), mode='wb') as file:
                file.write(byted)

    async def _dump_blosc_component_async(self, ix, component, ext, folder,
                                          index_to_name=None, i8_encoding_mode=None):
        """ Dump data from data_items on disk in specified folder asynchronously.
        ...
        ext : str
            extension of data file.
        folder : str
            folder to dump data-items in. Note that each data item is dumped
            in its separate subfolder inside the supplied folder.
        i8_encoding_mode: str, int, or dict
            contains mode of encoding to int8
        """
        # create directory if does not exist
        ...
        if ext == 'blk':
            ...
            _ = await encode_dump_array_async(self.get(ix, component),
                                              component_folder, 'data.blk', mode)

        elif ext == 'pkl':
            byted = pickle.dumps(self.get(ix, component))
            async with aiofiles.open(os.path.join(component_folder, 'data.pkl'), mode='wb') as file:
                _ = await file.write(byted)

    @inbatch_parallel(init='_init_dump', post='_post_default', target='for', update=False)
    def _dump_sync(self, ix, dst, components=None, index_to_name=None, i8_encoding_mode=None):
        ...
        for component in components:
            if component in ['images']:
                ...
                ext = 'blk'
            self._dump_blosc_component_sync(ix, component,
                                            ext, index_to_name,
                                            i8_encoding_mode)

    @inbatch_parallel(init='_init_dump', post='_post_default', target='async', update=False)
    async def _dump_async(self, ix, dst, components=None, index_to_name=None, i8_encoding_mode=None):
        """ Dump chosen ``components`` of scans' batcn in folder ``dst`` in specified format.
        ...
        for component in components:
            if component in ['images']:
                ...
                ext = 'blk'
            _ = await self._dump_blosc_component_async(ix, component,
                                                       ext, index_to_name,
                                                       i8_encoding_mode)

    @inbatch_parallel(init='_init_dump', post='_post_default', target='for', update=False)
    def _dump_dicom(self, ix, dst, component, index_to_name=None, intercept=0, slope=1):
        """ Dump data in dicom format.
        ...
        ix : str
            index of item in batch.
        dst : str
            path to directory where all data will be dumped.
        component : str
            name of component to dump.
        index_to_name : callable or None
            function that maps index to name.
        intercept : float
            interception argument. Default is 0.
        slope : float
            slope argument. Default is 1.
        """
        item_pos = self.index.get_pos(ix)
        item_dir = os.path.join(dst, ix if index_to_name is None else index_to_name(ix))
        dump_dicom(self.get(ix, component), item_dir, self.spacing[item_pos, :],
                   self.origin[item_pos, :], intercept, slope)

        return self

    @action
    def dump(self, dst, fmt='blosc', components=None, index_to_name=None, i8_encoding=None, **kwargs):
        """ Dump chosen ``components`` of scans' batcn in folder ``dst`` in specified format.
        ...
        sync : bool
            whether to use asyncio interface for dumping or not. Default is False,
            meaning that asyncio will be used.
        ...
        if fmt == 'blosc':
            if kwargs.pop('sync', False):
                return self._dump_sync(dst=dst ,components=components,
                                       i8_encoding_mode=i8_encoding,
                                       index_to_name=index_to_name, **kwargs)
            else:
                return self._dump_sync(dst=dst ,components=components,
                                       i8_encoding_mode=i8_encoding,
                                       index_to_name=index_to_name, **kwargs)
        elif fmt == 'dicom':
            return self._dump_dicom(dst=dst, component='images',
                                    index_to_name=index_to_name, **kwargs)
        elif fmt == 'raw':
            raise NotImplementedError("Dumping with fmt='raw' is not implemented yet!")
        return self
    ...
    def _init_images(self, **kwargs):
        ...
        """
        return [self.get(scan_id, 'images') for scan_id in self.indices]
    ...
    def rotate(self, index, angle, components='images', axes=(1, 2), random=True, **kwargs):
        ...
        """
        if not(isinstance(axes, (tuple, list, np.ndarray)) and len(axes) == 2):
            raise ValueError("Argument 'axes' must be array-like containing"
                             + " two int values representing axes"
                             + " associated with rotation plane. Got {}.")
        else:
    ...
    def make_xip(self, depth, stride=1, mode='max', projection='axial', padding='reflect', **kwargs):
        ...
        padding : str
            mode of padding that will be passed in numpy.padding function.
        """
        mode_values = ('max', 'min', 'mean', 'median')
        projection_values = ('axial', 'coronal', 'sagital')
        if not (isinstance(depth, INT_TYPES) and depth > 0):
            raise TypeError("Argument 'depth' must be positive integer. "
                            + "Got {}.".format(depth))

        if not (isinstance(stride, INT_TYPES) and stride > 0):
            raise TypeError("Arguemnt 'stride' must be positive integer. "
                            + "Got {}.".format(stride))

        if mode not in mode_values:
            raise ValueError("Argument 'mode' must be one of "
                             + "following strings: {}.".format(mode_values)
                             + "Got '{}'.".format(mode))

        if projection not in projection_values:
            raise ValueError("Argument 'projection' must be one of "
                             + "following strings: {}.".format(projection_values)
    ...
    def get_patches(self, patch_shape, stride, padding='edge', data_attr='images'):
        ...
        """
        if not np.all(self.images_shape == self.images_shape[0]):
            raise ValueError("Loaded images must be of equal size"
    ...
    def gaussian_filter(self, index, sigma, component='images', order=0, mode='reflect', cval=0.0, truncate=4.0, **kwargs):
        """ Apply gaussian filter to data component of batch.

        Parameters
        ----------
        sigma : float
            standart error for gaussian distribution.
        component : str
            name of data component. Default is 'images'.
    ...
    def additive_gaussian_noise(self, index, loc=0.0, scale=0.1, component='images', **kwargs):
        """ Add gaussian noise to data component of batch.

        Parameters
        ----------
        loc : float
            mean of gaussian distribution. Default is 0.0.
        scale : float
            scale of gaussian distribution. Default is 0.1.
        component : str
            name of data component. Default is 'images'.
        """
    ...
    @action
    @inbatch_parallel(init='indices', post='_post_components', targets='threads', new_batch=True)
    def salt_and_pepper(self, index, p=0.1, value=0.0, q=None, size_percent=None,
                        size=None, coarse=False, component='images', **kwargs):
        """ Apply salt and pepper augmentation to components of batch.
        ...
        q : float
            ...
            Used only if 'coarse == True'. Default is 1.
        ...
        components : tuple(str) or str
            components to apply salt_and_pepper augmentation.
        """
        data = self.get(index, component)
        out_data = salt_and_pepper(data, p=p, q=q, size_percent=size_percent,
                                   size=size, insert_value=value, coarse=coarse)
        return {component: out_data}

    @action
    @inbatch_parallel(init='indices', post='_post_components', target='threads', new_batch=True)
    def fft(self, index, component='images', axes=(0, 1, 2), norm=True, **kwargs):
        """ Apply fast fourier transform to data component of batch.

        Parameters
        ----------
        component : str
            data component of batch. Default is 'images'.
        axes : tuple(int)
            axes along which to apply fast fourier transform.
        norm : bool
            whether to take abs() of the result or raw output.
        """
    ...
    def ifft(self, index, component='images', axes=(0, 1, 2), norm=True, **kwargs):
        """ Apply inverse of fast fourier transform to data component of batch.
