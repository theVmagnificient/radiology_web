...
class Config:
    ...
    def _get(self, variables, config=None, **kwargs):
        ...
        if unpack:
            ret_vars = Config(ret_vars[0]) if isinstance(ret_vars[0], dict) else ret_vars[0]
        else:
            ret_vars = tuple(Config(value) if isinstance(value, dict) else value for value in ret_vars)
        return ret_vars
