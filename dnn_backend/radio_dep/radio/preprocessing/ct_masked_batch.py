...
from .exceptions import assert_component_is_loaded
...
class CTImagesMaskedBatch(CTImagesBatch):
    ...
    def nodules_to_df(self, nodules):
        ...
        return pd.DataFrame({'source_id': self.indices[nodules.patient_pos],
                             ...
                             'confidence': nodules.confidence[:,0]},
                            columns=columns)
    ...
    def get_pos(self, data, component, index):
        ...
        if data is None:
            ...
            if component in ['images', 'masks']:
                return slice(self.lower_bounds[ind_pos],
                             self.upper_bounds[ind_pos])
            else:
    ...
    @property
    @assert_component_is_loaded('masks')
    def masks_shape(self):
        """ Get shapes for masks of all 3d scans in CTImagesBatch.

        Returns
        -------
        ndarray(num_items, 3)
            shapes of masks in batch.
        """
    ...
    @action
    @assert_component_is_loaded('nodules')
    def create_mask(self, mode='ellipsoid'):
        ...
        for more details.
        """
        if mode not in ('ellipsoid', 'cuboid'):
            raise ValueError("Argument 'mode' must have one of following values "
                             + "('cuboid', 'ellipsoid'). Got '{}'.".format(mode))
    ...
    def rotate(self, index, angle, components='images', axes=(1, 2), random=True, **kwargs):
        ...
        if self.nodules is not None:
            out_dict['nodules'] = self._rotate_nodules(index, angle,
                                                       axes, out_size)
        return out_dict

    def _post_components(self, list_of_dicts, new_batch=True, **kwargs):
        """ Post function for components assembly.

        Simply calls _post_components from base class and after
        that performs assembly of nodules component.

        Parameters
        ----------
        list_of_dicts : list
            list with dicts. Each element in list is dictionary
            {'component_name': value, ...}. Dictionary may contain only those
            components that are supposed to be transformed.
        new_batch : bool
            whether to create new batch or not.

        Returns
        -------
        CTIMagesMaskedBatch
            batch with transformed components.
        """
        batch = super()._post_components(list_of_dicts, new_batch=new_batch, **kwargs)
        if not isinstance(batch.nodules, np.recarray) and batch.nodules is not None:
            batch.nodules = np.rec.array(batch.nodules)
        elif batch.nodules is not None:
            batch._refresh_nodules_info()
        else:
            batch.nodules = None
        return batch

    def sample_random_crops_positions(self, num_crops, crop_size, histo=None):
        """ Sample random nodules positions in CTImagesBatchMasked.

        Samples random crops positions in ndarray. Each crop have shape
        defined by `crop_size`. If size of patients' data along z-axis
        is not the same for different patients, NotImplementedError will be raised.

        Parameters
        ----------
        num_crops : int
            number of crops to sample from dataset.
        crop_size : ndarray(3, )
        ...
        ndarray
            ...
            crops, 2nd points out start positions (integers) of crops
            in batch `skyscraper`.
        """
        all_indices = np.arange(len(self))
        sampled_indices = np.random.choice(all_indices, num_crops, replace=True)

        offset = np.zeros((num_crops, 3))
        ...
        samples = sampler(size=num_crops) * (data_shape - crop_size)
    ...
    @action
    @assert_component_is_loaded('nodules')
    def sample_nodules(self, batch_size, crop_size=(32, 64, 64), share=0.8, variance=None,        # pylint: disable=too-many-locals, too-many-statements
        ...
        share : float
            ...
            if input CTImagesMaskedBatch contains less cancer
        ...
        if batch_size - cancer_n > 0:
            ...
            random_nodules, random_indices = (
                self.sample_random_crops_positions(batch_size - cancer_n,
                                                   crop_size, histo=histo)
                )
        ...
        images = get_nodules_numba(self.images, nodules_st_pos, crop_size)
        ...
        if mask_shape is not None:
            scale_factor = np.asarray(mask_shape) / np.asarray(crop_size)
        ...
        # build nodules' batch
        bounds = np.arange(batch_size + 1) * crop_size[0]
        ...
        nodules_batch._init_data(images=images, bounds=bounds,
                                 spacing=crops_spacing,
                                 origin=crops_origin, masks=masks)  # pylint: disable=protected-access
    ...
    @action
    def sample_dump(self, dst, n_iters, crop_size=(32, 64, 64), batch_size=20, share=0.8, **kwargs):
        ...
        **kwargs : dict
        ...
        for _ in range(n_iters):
            nodules = self.sample_nodules(batch_size=batch_size,
                                          crop_size=crop_size,
                                          share=share, **kwargs)
    ...
    @inbatch_parallel(init='_init_load_blosc', post='_post_components',
                      target='for', new_batch=False)
    def _load_blosc_sync(self, ix, *args, **kwargs):
        """ Read scans from blosc and puth them into batch components.
        ...
        **kwargs
            components : tuple
                ...
                that shoud be loaded into batch.
        ...
        for component in kwargs['components']:
            if component == 'nodules':
                continue
            ext = 'blk' if component in ['images', 'masks'] else 'pkl'
            self._load_blosc_component_sync(ix, component, ext)

        nodules_path = os.path.join(self.index.get_fullpath(ix),
                                    'nodules', 'data.pkl')

        if not (os.path.exists(nodules_path) and 'nodules' in kwargs['components']):
            return {}

        with open(nodules_path, mode='rb') as file:
            byted = file.read()
            nodules = pickle.loads(byted)

        nodules.patient_pos = self.index.get_pos(ix)
        return {'nodules': nodules}

    @inbatch_parallel(init='_init_load_blosc', post='_post_components',
                      target='async', new_batch=False)
    async def _load_blosc_async(self, ix, *args, **kwargs):
        ...
        **kwargs
        ...
        for component in kwargs['components']:
            if component == 'nodules':
                continue
            ext = 'blk' if component in ['images', 'masks'] else 'pkl'
            _ = await self._load_blosc_component_async(ix, component, ext)
    ...
    @inbatch_parallel(init='_init_dump', post='_post_default', target='for', update=False)
    def _dump_sync(self, ix, dst, components=None, fmt='blosc', index_to_name=None, i8_encoding_mode=None):
        ...
        if fmt != 'blosc':
            raise NotImplementedError('Dump to {}'.format(fmt)
        ...
        for component in components:
            ...
            if component in ['images', 'masks']:
                ...
                ext = 'pkl'
            self._dump_blosc_component_sync(ix, component, ext,
                                            dst, index_to_name,
                                            i8_encoding_mode)
        if 'nodules' in components:
            data = self.nodules[self.nodules.patient_pos == self.index.get_pos(ix)]
            byted = pickle.dumps(data)
            nodules_folder = os.path.join(dst, ix, 'nodules')
            if not os.path.exists(nodules_folder):
                os.makedirs(nodules_folder)

            with open(os.path.join(nodules_folder, 'data.pkl') , 'wb') as file:
                file.write(byted)


    @inbatch_parallel(init='_init_dump', post='_post_default', target='async', update=False)
    async def _dump_async(self, ix, dst, components=None, fmt='blosc', index_to_name=None, i8_encoding_mode=None):
        """ Dump chosen ``components`` of scans' batcn in folder ``dst`` in specified format.
        ...
        for component in components:
            ...
            if component in ['images', 'masks']:
                ...
                ext = 'pkl'
            _ = await self._dump_blosc_component_async(ix, component, ext,
                                                       dst, index_to_name,
                                                       i8_encoding_mode)
        if 'nodules' in components:
            data = self.nodules[self.nodules.patient_pos == self.index.get_pos(ix)]
            byted = pickle.dumps(data)
            nodules_folder = os.path.join(dst, 'nodules')
            if not os.path.exists(nodules_folder):
                os.makedirs(nodules_folder)

            with open(os.path.join(nodules_folder, ix, 'data.pkl') , 'wb') as file:
                file.write(byted)
    ...
    @action
    def make_xip(self, depth, stride=1, mode='max',
                 projection='axial', padding='reflect', **kwargs):
        ...
        batch = super().make_xip(stride=stride, depth=depth,
    ...
    def threshold_mask(self, threshold=0.35):
        ...
        if threshold < 0 or threshold > 1:
            raise ValueError("Argument 'threshold' must be float value "
                             + "from [0, 1] interval. "
    ...
    @action
    def predict_on_scan(self, model, strides=(16, 32, 32),
                        crop_shape=(32, 64, 64), batch_size=4,
        ...
        model : str or callable
            name of model that will be used for predictions or callable.
        ...
        elif callable(model) and model_type == 'callable':
            _model = model
        else:
            raise ValueError("Argument 'model' must be str or callable. "
                             + " If callable then 'model_type'"
                             + " argument must be 'callable'")
        crop_shape = np.asarray(crop_shape).reshape(-1)
        strides = np.asarray(strides).reshape(-1)
        
        if targets_mode not in ('segmentation', 'classification', 'regression'):
            raise ValueError("Argument 'targets_mode' must be one of " +
                             " 'classification', 'regression' or 'segmentation'. " +
                             " Got '{}' instead.".format(targets_mode))
        ...
        for i in iterations:
            ...
            if model_type == 'tf':
                _prediction = _model.predict(
                    feed_dict={'images': patches_arr[i: i + batch_size, ...]}
                )
            ...
            elif targets_mode == 'regression':
                current_prediction = create_mask_reg(current_prediction[:, :3],
                                                     current_prediction[:, 3:6],
                                                     current_prediction[:, 6],
                                                     crop_shape, 0.01)

            current_prediction = current_prediction.reshape(current_prediction.shape[0],
                                                            *crop_shape)
    ...
    def unpack(self, component='images', **kwargs):
        ...
        data_format : str
            ...
            channels dimension: right after batch dimension
            or after all spatial axes.
        kwargs : dict
    ...
    @assert_component_is_loaded('masks')
    @assert_component_is_loaded('images')
    def regression_targets(self, threshold=10, **kwargs):
        ...
        ndarray(batch_size, 7)
            ...
            and label(1 for cancerous and 0 for non-cancerous).
            Note that in case of non-cancerous crop first 6 column
        ...
        for item_pos, _ in enumerate(self.indices):
            ...
            nodule_sizes = (item_nodules.nodule_size
                            / self.spacing[item_pos, :]
                            / self.images_shape[item_pos, :])

            nodule_centers = (item_nodules.nodule_center
                              / self.spacing[item_pos, :]
                              / self.images_shape[item_pos, :])
    ...
    def make_data_tf(batch, model=None,
                     mode='segmentation', is_training=True, **kwargs):
    ...
    def show_info(self, index_visualize_len=10):
        """ Print batch components' content as table.

        Parameters
        ----------
        index_visualize_len : int
            length of visible part of batch indices if they are too long. Default is 10.
        """
        tab_data = []
        for e, index in enumerate(self.indices):
            tab_data.append(['...' + str(index[-index_visualize_len:]),
                            tuple(self.images_shape[e, :].tolist()),
                            '+' if getattr(self, 'images') is not None else '-',
                            '+' if getattr(self, 'masks') is not None else '-',
                            format_spacing(self.spacing[e, :].tolist()),
                            format_origin(self.origin[e, :].tolist())
                            ])

        print(tabulate(tab_data,
                       headers=['Index:', 'Shape:', 'Images:', 'Masks:', 'Spacing:', 'Origin:'],
